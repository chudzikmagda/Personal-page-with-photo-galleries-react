...
const GALLERIES = [
  ...
  {
    key: 'projectsWomensIceHockeyCracovia',
    path: 'projects/womens-ice-hockey-cracovia',
  },
...
const GALLERY_METADATA_OUTPUT_DIR = path.join(dirname, 'app/shared/metadata');
...
const getMetadataFileName = (galleryKey) =>
  `${galleryKey.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}.ts`;

const getGeneratedTimestamp = () => new Date().toISOString().replace('T', ' ').slice(0, 19);
...
const writeGalleryMetadataFile = (galleryKey, galleryItems) => {
  ...
  const outputContent = `// Auto-generated gallery image metadata — ${generatedAt}\nimport type { GalleryItem } from '../types/gallery.types';\n\nexport const ${varName}: GalleryItem[] = ${JSON.stringify(
    galleryItems,
    null,
    2
  )};\n`;
