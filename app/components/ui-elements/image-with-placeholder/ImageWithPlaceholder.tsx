...
const ImageWithPlaceholder: React.FC<ImageWithPlaceholderProps> = ({
  ...
  const [shouldRenderFullImage, setShouldRenderFullImage] = useState(
    loading !== ImageLoading.Lazy || !hasPlaceholder
  );
        ...
        <Image
          ...
          className={
            imageStyles?.className
              ? `${imageStyles.className} ${styles['fullsize-image']}`
              : styles['fullsize-image']
          }
          style={{
            ...imageStyles?.style,
            opacity: hasPlaceholder && showPlaceholder ? 0 : 1,
          }}
          alt={alt ?? ''}
          loading={loading}
          onLoad={handleLoad}
